...
`)},
                ...
                UNION ALL
                  ...
                  SELECT 
                    'DELETE' AS __op__,
                    ${E.map(({column_name:p,data_type:B,udt_name:ee})=>p===i?`prev."${p}" AS "${p}"`:`NULL${B==="USER-DEFINED"?`::${ee}`:""} AS "${p}"`).join(`,
`)},
                      ARRAY[]::text[] AS __changed_columns__
                  FROM prev
                ...
                UNION ALL
                  ...
                  SELECT 
                    'UPDATE' AS __op__,
                    ${E.map(({column_name:p,data_type:B,udt_name:ee})=>p===i?`curr."${p}" AS "${p}"`:`CASE 
                              WHEN curr."${p}" IS DISTINCT FROM prev."${p}" 
                              THEN curr."${p}"
                              ELSE NULL${B==="USER-DEFINED"?`::${ee}`:""}
                              END AS "${p}"`).join(`,
`)},
    ...
    `,[t])).rows.map(s=>({table_name:s.table_name,schema_name:s.schema_name,table_oid:s.table_oid,schema_oid:s.schema_oid}))}async function Ee(e,t,n){let s=t.filter(r=>!n.has(`${r.schema_oid}_${r.table_oid}`)).map(r=>`
      ...
      `).join(`
`);s.trim()!==""&&await e.exec(s),t.map(r=>n.add(`${r.schema_oid}_${r.table_oid}`))}var Z=(e,t)=>{for(let n of e)n(t)},Rt=(e,t)=>{for(let n of e)n(t)};0&&(module.exports={live});
