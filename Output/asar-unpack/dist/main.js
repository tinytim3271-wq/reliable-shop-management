...
var isPackaged = import_electron.app.isPackaged;
var resourcesRoot = isPackaged ? process.resourcesPath : import_node_path.default.resolve(__dirname, "..", "..");
var apiEntry = isPackaged ? import_node_path.default.join(resourcesRoot, "api-server", "dist", "index.mjs") : import_node_path.default.join(resourcesRoot, "artifacts", "api-server", "dist", "index.mjs");
var apiResourcesDir = isPackaged ? import_node_path.default.join(resourcesRoot, "api-server") : import_node_path.default.join(resourcesRoot, "artifacts", "api-server");
var frontendDir = isPackaged ? import_node_path.default.join(resourcesRoot, "frontend") : import_node_path.default.join(resourcesRoot, "artifacts", "mechanic-ledger", "dist", "public");
var migrationsDir = isPackaged ? import_node_path.default.join(resourcesRoot, "migrations") : import_node_path.default.join(resourcesRoot, "lib", "db", "drizzle");
var PREFERRED_PORT = 4317;
var apiProcess = null;
var apiPort = PREFERRED_PORT;
var mainWindow = null;
...
function startApiServer(port) {
  ...
  const aiBaseUrl = process.env.AI_INTEGRATIONS_OPENAI_BASE_URL ?? "http://127.0.0.1:11434/v1";
  const aiApiKey = process.env.AI_INTEGRATIONS_OPENAI_API_KEY ?? "local-dev-placeholder";
  const env = {
    ...process.env,
    // Run the api-server's ESM bundle using Electron's bundled Node so the
    // packaged app needs no separate Node install.
    ELECTRON_RUN_AS_NODE: "1",
    NODE_ENV: "production",
    APP_RUNTIME: "desktop",
    PORT: String(port),
    PGLITE_DATA_DIR: import_node_path.default.join(userData, "pglite-data"),
    RSS_STORAGE_DIR: import_node_path.default.join(userData, "rss-storage"),
    RSS_SESSION_SECRET_FILE: import_node_path.default.join(userData, "session-secret"),
    PGLITE_MIGRATIONS_DIR: migrationsDir,
    RSS_FRONTEND_DIR: frontendDir,
    AI_INTEGRATIONS_OPENAI_BASE_URL: aiBaseUrl,
    AI_INTEGRATIONS_OPENAI_API_KEY: aiApiKey
  };
  delete env.DATABASE_URL;
  delete env.DB_DRIVER;
  delete env.STORAGE_BACKEND;
  if (isPackaged) {
    const apiNodeModules = import_node_path.default.join(apiResourcesDir, "node_modules");
    env.NODE_PATH = apiNodeModules;
  }
  const child = (0, import_node_child_process.spawn)(process.execPath, [apiEntry], {
    env,
    cwd: apiResourcesDir,
  ...
  child.on("exit", (code, signal) => {
    apiProcess = null;
    if (!isQuitting) {
      const errorDetail = errorOutput.slice(-500);
      const message = `The local server stopped unexpectedly (code ${code ?? "?"}, signal ${signal ?? "none"}).
...
function waitForHealth(port, timeoutMs = 6e4) {
  const start = Date.now();
  return new Promise((resolve, reject) => {
    const attempt = () => {
      const req = import_node_http.default.get(
        { host: "127.0.0.1", port, path: "/api/healthz", timeout: 2e3 },
        (res) => {
          res.resume();
    ...
    const retry = () => {
      if (Date.now() - start > timeoutMs) {
        reject(new Error("API server did not become healthy in time."));
      } else {
...
async function showConnectWindow() {
  const urls = getLanUrls(apiPort);
  const primary = urls[0] ?? `http://127.0.0.1:${apiPort}/`;
  let qrDataUrl = "";
  try {
    qrDataUrl = await import_qrcode.default.toDataURL(primary, { width: 320, margin: 1 });
  } catch {
  }
  const listHtml = urls.length ? urls.map((u) => `<li><code>${u}</code></li>`).join("") : "<li>No local network address detected. Connect this PC to Wi-Fi or a LAN.</li>";
  const html = `<!doctype html><html><head><meta charset="utf-8" />
<title>Connect a tablet</title>
<style>
  body { font-family: "Segoe UI", Arial, sans-serif; margin: 0; padding: 24px; background: #0f172a; color: #e2e8f0; }
  h1 { font-size: 18px; margin: 0 0 4px; }
  p { color: #94a3b8; margin: 0 0 16px; font-size: 13px; }
  img { display: block; margin: 0 auto 16px; background: #fff; padding: 8px; border-radius: 8px; }
  ul { list-style: none; padding: 0; margin: 0; }
  li { background: #1e293b; padding: 8px 12px; border-radius: 6px; margin-bottom: 6px; font-size: 13px; }
  code { color: #7dd3fc; }
</style></head><body>
  <h1>Connect a tablet</h1>
  <p>In the Android companion app, enter one of these shop server addresses, or scan the code below.</p>
  ${qrDataUrl ? `<img src="${qrDataUrl}" width="320" height="320" alt="QR code for the shop server address" />` : ""}
  <ul>${listHtml}</ul>
</body></html>`;
...
function createMainWindow() {
  mainWindow = new import_electron.BrowserWindow({
    ...
    height: 860,
    show: false,
    title: "Reliable Shop Systems",
    webPreferences: { contextIsolation: true, nodeIntegration: false }
  });
  import_electron.Menu.setApplicationMenu(buildMenu());
  void mainWindow.loadURL(`http://127.0.0.1:${apiPort}/dashboard`);
...
if (!gotSingleInstanceLock) {
  ...
  void import_electron.app.whenReady().then(async () => {
    try {
      apiPort = await findFreePort(PREFERRED_PORT);
...
${String(err)}`
  ...
  import_electron.app.on("activate", () => {
    if (import_electron.BrowserWindow.getAllWindows().length === 0 && apiProcess) {
      createMainWindow();
    }
  });
  ...
  import_electron.app.on("before-quit", (event) => {
    ...
    isQuitting = true;
    void shutdownApi().then(() => import_electron.app.quit());
  });
}
